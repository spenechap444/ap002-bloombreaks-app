...
class textDB(PostgresDB):
    ...
    def fetch_sms_subscribers(self):
        query = 'SELECT * FROM records_api_dbo.fetch_user_messaging_by_status(%s);'

        result = self.fetch_proc(query, 'ACTIVE')
        if isinstance(result, list):
            return [row[0] for row in result]
        return []

    def store_sms_subscriber(self, text_subscriber):
        query = 'CALL records_api_dbo.aip_user_messaging_status_insert(%s, %s, %s);'
